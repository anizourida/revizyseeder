...
class GenerateStandardVocabularyQuestionsJob implements ShouldQueue
{
    ...
    public function handle(QuestionStudioService $service): void
    {
        ...
        Log::info('raiida.admin_mutation.job.generate_vocab_standard_questions.started', $audit);

        try {
            $summary = $service->batchGenerateAndPublishStandard($this->options);
        } catch (Throwable $exception) {
            Log::error('raiida.admin_mutation.job.generate_vocab_standard_questions.failed', $audit + [
        ...
        Log::info('raiida.admin_mutation.job.generate_vocab_standard_questions.completed', $audit + $summary);
    }
}
